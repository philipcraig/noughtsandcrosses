...
newtype Value =
  Value Int
  deriving (Eq, Ord, Enum, Bounded, Num, Real, Integral, Show, Read)

-- compute best move using some search function
-- undefined for terminal positions
searchMove
  :: Transitions s' l
  => (Value -> Value -> Int -> s' -> Value) -> Int -> s' -> (l, Value)
searchMove abSearch depth s' = cmx (minBound + 1) first (transitions s')
  where
    ...
    cmx !alpha best ((l, s):rest) = cmx alpha' best' rest
      where
        !v = -abSearch (-maxBound) (-alpha) (depth - 1) s
        !alpha' =
          if v > alpha
            then v
            else alpha
...
alphaBeta valf alpha' beta depth s
  ...
  where
    ...
    cmx !alpha (p:ps)
      ...
      where
        a' = -alphaBeta valf (-beta) (-alpha) (depth - 1) p
